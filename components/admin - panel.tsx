...
export function AdminPanel() {
  const [auth, setAuth] = useState<AdminState>({ authenticated: false, password: "" });
  ...
  async function loadData() {
    ...
    if (categoriesData.length && !productCategory) {
  ...
  async function handleLogin(e: React.FormEvent) {
    e.preventDefault();
    if (auth.password === process.env.NEXT_PUBLIC_FAKE_ADMIN_PASSWORD) {
      setAuth({ ...auth, authenticated: true });
      return;
    }

    // fallback if env not exposed in client
      ...
      <div className="admin-grid">
        <div className="admin-card">
          ...
          <form className="form-grid" onSubmit={addCategory}>
            ...
            />
            <button className="btn" type="submit" disabled={loading}>Add Category</button>
          </form>
        </div>
        ...
        <div className="admin-card">
          ...
          <form className="form-grid" onSubmit={addProduct}>
            ...
            <button className="btn" type="submit" disabled={loading}>Add Product</button>
          </form>
        </div>
      </div>
          ...
          {categories.map((category) => (
            <div className="list-item" key={category.id}>
              ...
              <div className="list-actions">
                <button className="btn danger" onClick={() => deleteCategory(category.id)}>Delete</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="list-grid">
          {products.map((product) => (
            <div className="list-item" key={product.id}>
              <div className="list-left">
                <img className="thumb" src={product.image_url || "/placeholder-food.jpg"} alt={product.name} />
                <div>
                  <strong>{product.name}</strong><br />
                  <span className="muted">
                    {categoryMap[product.category_id] || "Unknown"} • ${Number(product.price).toFixed(2)}
                  </span>
                </div>
              </div>
              <div className="list-actions">
